...
async function testBackend() {
  ...
  try {
    ...
    const dbStatus = await axios.get(\/api/db-status);
    console.log('DB Status:', dbStatus.data);

    console.log('\n--- Testing /api/parts ---');
    const parts = await axios.get(\/api/parts);
    console.log('Parts Count:', parts.data.length);

    console.log('\n--- Testing /api/store-availability ---');
    const storeAvail = await axios.get(\/api/store-availability);
    console.log('Store Availability Rows:', storeAvail.data.length);

    console.log('\n--- Testing /smart/builder/recommend ---');
    const recommendation = await axios.post(\/smart/builder/recommend, {
      budget: 85000,
      use_case: 'gaming',
      preferred_brand: '',
    ...
    if (recommendation.data.length > 0) {
      ...
      const postBuild = await axios.post(\/api/builds, savePayload);
